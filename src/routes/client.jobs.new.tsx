...
function CreateJob() {
  ...
  const escrowAmount = useMemo(() => paymentNumber, [paymentNumber]);
  const [taskCountInput, setTaskCountInput] = useState(String(items.length));
  const [taskCountFocused, setTaskCountFocused] = useState(false);

  useEffect(() => {
    if (!taskCountFocused) {
      setTaskCountInput(String(items.length));
    }
  }, [items.length, taskCountFocused]);

  const createDefaultTask = (id: number): Draft => ({
    id,
    title: "",
    instructions: "",
    required: ["photo"],
  ...
  const setTaskCount = (nextCount: number) => {
    const count = Math.max(1, Math.trunc(nextCount));
    setItems((prev) => {
      ...
      if (count > prev.length) {
        ...
        return [
          ...prev,
          ...Array.from({ length: count - prev.length }, (_, index) => createDefaultTask(nextId + index)),
        ];
      }
            ...
            <div className="grid gap-4">
              ...
              <div className="rounded-2xl border border-dashed border-border bg-muted/30 p-4">
                ...
                <div className="mt-6 rounded-2xl border border-border bg-card p-4">
                  <div className="mb-3">
                    <p className="text-base font-semibold">Task Count</p>
                    <p className="text-sm text-muted-foreground">Specify how many checklist items this job requires</p>
                  </div>
                  <label className="block">
                    <input
                      ...
                      onFocus={() => setTaskCountFocused(true)}
                      onBlur={() => {
                        setTaskCountFocused(false);
                        const count = Number(taskCountInput);
